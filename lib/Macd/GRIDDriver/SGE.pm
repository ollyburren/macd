package Macd::GRIDDriver::SGE;

use strict;
use base ('Macd::GRIDDriver');
use Macd::Utils qw/:DEFAULT/;
use Cwd qw/abs_path/;

## must implement submit and check_finished methods 
## see parent class Macd::GRIDDriver for details

sub submit{
	my $self = shift;
	my ($cmd,$logdir,$env_settings)=@_;
	if(! $cmd){
		debug("No command given for submission !");
		return (undef,-1);
	}
	if(! -d $logdir){
		if(!mkpath_wrapper($logdir)){
			debug("Could not create log dir $logdir");
			return (undef,-1);
		}
	}
  ## make sure that we use absolute path for logfile  
  my $ldir = abs_path($logdir);
